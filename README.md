...
Configuration managed via [GokuRakuJoudo](https://github.com/yqrashawn/GokuRakuJoudo).

**File structure:**

- **Source**: `.config/karabiner.edn` (EDN format, human-friendly)
- **Generated**: `.config/karabiner/karabiner.json` (auto-generated by Goku)
- **Docs**: `.config/karabiner/` (README, guides)

**Note:** Karabiner configs are *not* symlinked by Stow. The repo contains default configs, but copy them to `~/.config/` for per-machine customization:

```bash
cp ~/.dotfiles/.config/karabiner.edn ~/.config/karabiner.edn
cp ~/.dotfiles/.config/karabiner/karabiner.json ~/.config/karabiner/karabiner.json
```

**Generate config:**

```bash
goku  # Run once to generate JSON from EDN
```

See [.config/karabiner.edn](.config/karabiner.edn) for the source config and [.config/karabiner/README.md](.config/karabiner/README.md) for hotkey reference.

**Key highlights:**

- **Caps Lock** → Hyper key (hold) / Escape (tap)
- **Hyper+Key** → Launch applications (Hyper+W=Chrome, Hyper+T=iTerm, etc.)
- **Hyper+Cmd+IJKL** → Navigation arrows
- **Fn+F-keys** → Standard function keys (except in IDEs)
- **j+k** → Escape
